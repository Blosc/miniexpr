 ...
 *
 * Demonstrates logical behavior of &, |, ^, and ~ when used with boolean arrays.
 * For boolean inputs, these operators follow NumPy-style logical semantics.
 */
...
int main() {
    ...
    printf("----------------------------------------\n");
    {
        ...
        expr = NULL;
        if (me_compile("~a", vars_a, 1, ME_BOOL, &err, &expr) != ME_COMPILE_SUCCESS) {
            printf("Compilation error at position %d\n", err);
            return 1;
        }
        const void *ptrs_not[] = {a};
        ME_EVAL_CHECK(expr, ptrs_not, 1, result, N);
        print_bool_row("~a", result);
        me_free(expr);
    }

    printf("\nExample 2: Combine comparison masks with &\n");
    printf("------------------------------------------\n");
    {
        ...
        if (me_compile("((o0 > 0.5) & (o1 > 10000))", vars, 2, ME_BOOL, &err, &expr) != ME_COMPILE_SUCCESS) {
            printf("Compilation error at position %d\n", err);
            return 1;
        }
        ...
        printf("  idx |  o0  |  o1   | (o0 > 0.5) & (o1 > 10000)\n");
        printf("  ----+------+-------+-------------------------\n");
        for (int i = 0; i < N; i++) {
            printf("  %3d | %4.2f | %5d | %s\n",
