/*
 * Benchmark a heavy transcendentals chain for float32/float64.
 */
    ...
    for (int it = 0; it < iterations; it++) {
        if (info->dtype == ME_FLOAT32) {
            ...
            for (int i = 0; i < nitems; i++) {
                float x = a[i];
                float ax = fabsf(x);
                float val = expf(x) + tanhf(x) + log1pf(ax) + sqrtf(ax) + expm1f(x);
                o[i] = logf(val);
            }
            sink += o[nitems - 1];
        } else {
            ...
            for (int i = 0; i < nitems; i++) {
                double x = a[i];
                double ax = fabs(x);
                double val = exp(x) + tanh(x) + log1p(ax) + sqrt(ax) + expm1(x);
                o[i] = log(val);
            }
            sink += o[nitems - 1];
        }
...
static void benchmark_dtype(const dtype_info_t *info, const int *blocks, int nblocks) {
    ...
    int err = 0;
    me_expr *expr = NULL;
    const char *expr_text = "log(exp(x) + tanh(x) + log1p(abs(x)) + sqrt(abs(x)) + expm1(x))";
